...
public class 다익스트라_최단경로 {
	...
	private static void dijkstra() {
		...
		while(!pq.isEmpty()){//한 정점씩 알아가면서 제일 가까운 거리로만감
			...
			if(visit[currentVertaxNumber]==false){
				...
				for(int[] nextVertaxInfo : edgeList[currentVertaxNumber]){//매달려있는 정점들에대해
					if(visit[nextVertaxInfo[0]]==false) {//아직 최단거리를 모르는 정점이라면 
