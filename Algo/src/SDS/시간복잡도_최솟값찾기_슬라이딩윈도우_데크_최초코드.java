 ...
 * 
 */
public class 시간복잡도_최솟값찾기_슬라이딩윈도우_데크_최초코드{
	static int N,L;
	static int[] series;
	...
	public static void main(String[] args) throws IOException {
		...
		series = new int[N+1];
		for(int i=1;i<=N;i++) series[i] = Integer.parseInt(st.nextToken());
		//처리
		slidingWindow();
		//출력
		...
		for(int i=1;i<=N;i++){ //i가 3일때부터 
			...
			while(!dq.isEmpty() && series[dq.peekLast()]>series[i]){
				//비어있으면 검사가 안되니까 isEmpty로 체크
				dq.pollLast();
			}
			dq.offerLast(i);//인덱스를 넣는다.
			bw.write(series[dq.peekFirst()]+" ");
		}
	}
