...
public class 산업스파이의편지 {
	...
	static HashSet<Integer> findPrime;
	...
	private static void dfs(int depth,String makeNumber){
		if(makeNumber.length()>0) {
			if(makeNumber.charAt(0)=='0') return;
		}
		if(depth==r){
			if(Integer.parseInt(makeNumber)>1 && !prime[Integer.parseInt(makeNumber)] && !findPrime.contains(Integer.parseInt(makeNumber))){ 
				findPrime.add(Integer.parseInt(makeNumber));
				sum++;
			}
			return;
		}
